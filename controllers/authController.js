...
//   try {
//     const message = await authService.validateToken(req);
...
const signup = async (req, res) => {
  try {
    const { full_name, email, username, password, profile_photo } = req.body;
    const result = await authService.signup(full_name, email, username, password, profile_photo);
    if (result.success) {
...
const sendOTP = async (req, res) => {
  try {
    ...
    const result = await authService.sendOTP(email, otp);
    if (result.success) {
      res.status(200).json({ message: result.message });
    } else {
      res.status(401).json({ error: result.error, message: result.message });
